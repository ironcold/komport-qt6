...
- **Meilenstein 7 — Retro-/Industrie-Zeichensatz-Übersetzung**
  (2026-09-09): `KomportCharset`-Klasse, byte-basierte Übersetzung
  zwischen rohem seriellem Bytestrom und Terminal-Emulation. Eingebaut:
  CP437, PETSCII; bewusst nicht umgesetzt: "Amiga" und CP437s
  0x00-0x1F-Grafikbereich (Abschnitt 6.3). Plus Nachtrag:
  benutzerdefinierte `*.charset`-Dateien ohne Code/Neubau (Abschnitt 1).
  Details, Review-Funde und Fixes in `TODO-ARCHIVE.md` Abschnitt 23
  (23.1 für den Custom-Charset-Nachtrag). PR #6 + #7 auf Codeberg, beide
  gemergt.
- **Meilenstein 6 — Internationalisierung (i18n)** (2026-09-11): alle
  sichtbaren String-Literale nutzen `tr()`, Qt6-`LinguistTools`
  eingebunden, vollständige deutsche Übersetzung. Plus Nachtrag: alle
  24 EU-Amtssprachen per lokalem Gemma4 vorbereitet, 23 davon
  vollständig, Irisch bewusst als unübersetztes Skeleton zurückgestellt
  (durchgehend fehlerhafte Modellausgabe, dem Nutzer vorgelegt statt
  geraten). Details, Review-Funde und Fixes in `TODO-ARCHIVE.md`
  Abschnitt 22 (22.1 für den EU-Amtssprachen-Nachtrag). PR #8 + #9 auf
  Codeberg, beide gemergt.
...
Alle Punkte umgesetzt (inkl. Nachtrag: alle 24 EU-Amtssprachen, 23
vollständig übersetzt, Irisch bewusst als Skeleton zurückgestellt) und
über mehrere Codex/Gemma4-Review-Runden verifiziert — Details, Findings
und Fixes in `TODO-ARCHIVE.md` Abschnitt 22 (22.1 für den
EU-Amtssprachen-Nachtrag). PR #8 + #9 auf Codeberg, beide gemergt.
