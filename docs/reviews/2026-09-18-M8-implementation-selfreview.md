...
| A dedicated compile/object target with only the public session and transport headers, linking `Qt6::Core` (not the Widgets-linking test targets), builds | `tests/CMakeLists.txt`: target `komport_session_contract_check` compiles `session_contract_compile.cpp` and every implementation unit of the public surface - `itransport.cpp`, `transportconfiguration.cpp`, `sessioncontroller.cpp` - against `Qt6::Core` only; no `komport_core`, no `Qt6::Test`. Corrected 2026-09-18 after the transport-interface review: `transportconfiguration.cpp` was missing from the target, so the row's earlier wording overstated the proof | met |
